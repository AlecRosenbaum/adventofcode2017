...
def solution_part_one(arg):
    ...
    for line in arg.split("\n"):
        target, op, amt, _, cond, comp, val = line.split()
        if eval(f"{registers.get(cond, 0)} {comp} {val}"):
            if op == "inc":
                registers[target] = registers.get(target, 0) + int(amt)
            elif op == "dec":
                registers[target] = registers.get(target, 0) - int(amt)
    ...
    for line in arg.split("\n"):
        ...
        if eval(f"{registers.get(cond, 0)} {comp} {val}"):
            if op == "inc":
                ...
                registers[target] = registers.get(target, 0) - int(amt)
        if len(registers.values()):
            curr_max = max(curr_max, max(registers.values()))

    return curr_max


if __name__ == "__main__":
    puzzle_input = """utc dec -736 if p > -7
tn inc -876 if qlm == 4
uz dec 294 if l < 10
a inc -904 if me >= -7
tn inc 622 if ppl <= 6
